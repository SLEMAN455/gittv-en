...
on:
  schedule:
    # Chạy 2 lần/ngày vào 9h sáng và 9h tối (giờ UTC)
    - cron: '0 9,21 * * *'
...
jobs:
  update-playlist:
    ...
    # Giới hạn thời gian chạy an toàn là 45 phút
    timeout-minutes: 45
    ...
    steps:
      ...
      - name: Configure Run Mode
        run: |
          # Sử dụng file iptv_generator_optimized.py (đã nâng cấp lên High Quality)
      ...
      - name: Validate New Playlist
        ...
        run: |
          ...
          # THAY ĐỔI: Yêu cầu tối thiểu giảm xuống 30 kênh (do filter nghiêm ngặt hơn)
